...
unless(defined(&_FEATURES_H)) {
    ...
    if((defined (&_DEFAULT_SOURCE) || (!defined (&__STRICT_ANSI__)  && !defined (&_ISOC99_SOURCE)  && !defined (&_POSIX_SOURCE)  && !defined (&_POSIX_C_SOURCE)  && !defined (&_XOPEN_SOURCE)  && !defined (&_BSD_SOURCE)  && !defined (&_SVID_SOURCE)))) {
	undef(&_DEFAULT_SOURCE) if defined(&_DEFAULT_SOURCE);
	eval 'sub _DEFAULT_SOURCE () {1;}' unless defined(&_DEFAULT_SOURCE);
	undef(&_BSD_SOURCE) if defined(&_BSD_SOURCE);
	eval 'sub _BSD_SOURCE () {1;}' unless defined(&_BSD_SOURCE);
	...
	undef(&_FORTIFY_SOURCE) if defined(&_FORTIFY_SOURCE);
    }
    ...
    eval 'sub __GLIBC_MINOR__ () {19;}' unless defined(&__GLIBC_MINOR__);
