...
source "${PORTAGE_BIN_PATH:-/home/core/gentoo/usr/lib/portage/bin}/ebuild.sh"
...
install_qa_check_misc() {
	...
	if [[ -n ${PORTAGE_LOG_FILE} && -r ${PORTAGE_LOG_FILE} ]] ; then
		...
		f=$(cd "${PORTAGE_PYM_PATH}" ; $cat_cmd "${PORTAGE_LOG_FILE}" | \
			"${PORTAGE_PYTHON:-/home/core/gentoo/usr/bin/python}" "$PORTAGE_BIN_PATH"/check-implicit-pointer-usage.py || die "check-implicit-pointer-usage.py failed")
		if [[ -n ${f} ]] ; then
...
__dyn_package() {
	...
	PYTHONPATH=${PORTAGE_PYTHONPATH:-${PORTAGE_PYM_PATH}} \
		"${PORTAGE_PYTHON:-/home/core/gentoo/usr/bin/python}" "$PORTAGE_BIN_PATH"/xpak-helper.py recompose \
